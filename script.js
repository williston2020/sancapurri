...
// Manejar subida de múltiples imágenes a la galería
function handleMultipleGalleryImageUpload(event) {
    ...
    if (files.length > 0) {
            ...
            if (file && file.type.startsWith(\'image/\')) {
                ...
                reader.onload = function(e) {
                    ...
                    if (filesProcessed === files.length) {
                        localStorage.setItem(\'sancapurris_images\', JSON.stringify(imagesList));
                        loadGallery();
                        showNotification(\'¡Múltiples imágenes agregadas exitosamente a la galería! 🎉\');
                    }
                };
                reader.readAsDataURL(file);
            } else {
                showNotification(\'Algunos archivos seleccionados no son imágenes válidas y fueron omitidos.\', \'warning\');
                filesProcessed++;
                if (filesProcessed === files.length) {
                    localStorage.setItem(\'sancapurris_images\', JSON.stringify(imagesList));
                    loadGallery();
        ...
        showNotification(\'Por favor selecciona al menos un archivo de imagen válido\', \'error\');
    }
    event.target.value = \'\'; // Limpiar input
}
